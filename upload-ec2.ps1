...
if (Test-Path $jarFile) {
    Write-Host "   Enviando JAR ($((Get-Item $jarFile).Length / 1MB) MB)..." -ForegroundColor Cyan
    scp -i $keyPath $jarFile "${ec2Host}:/home/ubuntu/"
...
if (Test-Path $frontendDir) {
    $frontendSize = (Get-ChildItem $frontendDir -Recurse | Measure-Object -Property Length -Sum).Sum / 1MB
    Write-Host "   Enviando frontend ($frontendSize MB)..." -ForegroundColor Cyan
    scp -i $keyPath -r $frontendDir "${ec2Host}:/home/ubuntu/"
